...
# P10k already renders rich git status; adding another hook would duplicate
# work. When p10k is absent (for example inside a minimal SSH session) we fall
# back to `vcs_info` so the prompt still surfaces the current branch without
# extra plugins.
